    ...
    def sample(self, word_inputs, feature_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover, batch_label, mask, no_samples):
      ...
      #  outs = torch.clamp(outs, max = 18)
        ...
        for n in range(no_samples):
            ...
            #print("outs: ", idtask, outs.shape)
                if self.use_crf:
                    ...
                    #compute reward
                    #reward_list = []
                  # print("squeeze", tag_seq.squeeze().shape)
                    #for tag, gold_tag in zip(tag_seq.squeeze(),batch_label[idtask].squeeze()):
                        #tag_get =  self.data.label_alphabet[idtask].get_instance(tag.data.cpu().numpy()[0])
                        #gold_get = self.data.label_alphabet[idtask].get_instance(gold_tag.data.cpu().numpy()[0])
                        #print(tag_get,gold_get)
                    #    if int(tag.data.cpu().numpy()[0]) == int(gold_tag.data.cpu().numpy()[0]):
                    #        reward_list.append(1.0)
                    #    else:
                    #        reward_list.append(-1.0)
                    #all_rewards[idtask].append(reward_list)
        ...
        #print("AFTER ", len(all_samples_tag_sequences))
            ...
            #if self.average_batch:
            #total_loss = total_loss / batch_size
        return all_scores, all_samples_tag_sequences, all_rewards
